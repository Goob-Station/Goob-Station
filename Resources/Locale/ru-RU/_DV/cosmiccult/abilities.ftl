...
cosmicability-siphon-cultist-success = Ваши попытки поглотить энтропию охлаждают { CAPITALIZE(THE($target)) }.
cosmicability-siphon-success = Вы беззвучно поглощаете энтропию из { CAPITALIZE(THE($target)) }.
cosmicability-siphon-fail = Вам не удаётся вытянуть энтропию из { CAPITALIZE(THE($target)) }.
...
cosmicability-blank-success = Вы изгоняете разум { CAPITALIZE(THE($target)) } во космическую пустоту.
...
cosmicability-lapse-success = Вы разрываете физическую оболочку { CAPITALIZE(THE($target)) }.
