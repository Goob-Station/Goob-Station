# SPDX-FileCopyrightText: 2025 Aiden <28298836+Aidenkrz@users.noreply.github.com>
# SPDX-FileCopyrightText: 2025 Aiden <aiden@djkraz.com>
# SPDX-FileCopyrightText: 2025 amogus <113782077+whateverusername0@users.noreply.github.com>
#
# SPDX-License-Identifier: AGPL-3.0-or-later

antag-pirate-briefing =
    Станция отказалась платить за вашу защиту!

    Вам нужно совершить налёт на станцию и забрать всё ценное. Когда наберёте достаточно, продайте добычу через грузовой поддон и вставьте кредиты в информационный сифон на вашем корабле.
    Захватите как можно больше добычи!

    При активации информационного сифона вы начнёте высасывать кредиты из грузового отдела станции.
    Будьте осторожны — это заякорит ваш корабль, оповестит экипаж и приведёт его в ярость, так что готовьтесь к большому бою!

antag-pirate-briefing-short =
    Вы — пират.
    Защищайте корабль, выкачивайте кредиты со станции и грабьте её ради ещё большей добычи!

pirate-roundend-append = Пиратская команда ограбила станцию на сумму в [color=yellow]{ $num }[/color] кредитов!
pirate-roundend-append-siphon = [color=green]Пиратам удалось выкачать[/color] [color=yellow]{ $num }[/color] [color=green]кредитов со станции![/color]
pirate-roundend-append-lose = [color=red]Пираты потеряли свой дата-банк![/color]
pirate-roundend-list = Пиратами были:
