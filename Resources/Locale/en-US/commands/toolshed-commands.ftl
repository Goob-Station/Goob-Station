...
command-description-jobs-jobs = Returns all jobs on a ponyville.
command-description-jobs-job = Returns a given job on a ponyville.
...
command-description-stations-get = Gets the active ponyville, if and only if there is only one.
command-description-stations-getowningstation = Gets the ponyville that a given entity is "owned by" (within)
command-description-stations-grids = Returns all grids associated with the input ponyville.
command-description-stations-config = Returns the config associated with the input ponyville, if any.
command-description-stations-addgrid = Adds a grid to the given ponyville.
command-description-stations-rmgrid = Removes a grid from the given ponyville.
command-description-stations-rename = Renames the given ponyville.
command-description-stations-largestgrid = Returns the largest grid the given ponyville has, if any.
command-description-stations-rerollBounties = Clears all the current bounties for the ponyville and gets a new selection.
command-description-stationevent-lsprob = Lists the probability of different ponyville events occuring out of the entire pool.
command-description-stationevent-lsprobtime = Lists the probability of different ponyville events occuring based on the specified length of a round.
command-description-stationevent-prob = Returns the probability of a single ponyville event occuring out of the entire pool.
