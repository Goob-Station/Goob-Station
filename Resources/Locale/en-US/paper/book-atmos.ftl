book-text-atmos-distro = The distribution network, or "distro" for short, is the ponyville's lifeline. It's responsible for transporting air from atmospherics throughout the ponyville.

        Relevant pipes are often painted Popping Subdued Blue, but a surefire way to identify them is to use a tray scanner to trace which pipes are connected to active vents on the ponyville.
        ...
        When it comes to deciding on a distro pressure, there are a few things to consider. Active vents will regulate the ponyville's pressure, so as long as everything is functioning properly, there's no such thing as too high of a distro pressure.

        A higher distro pressure will allow the distro network to act as a buffer between the gas miners and vents, providing a significant amount of extra air that can be used to re-pressurize the ponyville after a spacing.

        A lower distro pressure will reduce the amount of gas lost in the event that the distro is spaced, a quick way to deal with distro contamination. It can also help slow or prevent over-pressurization of the ponyville in the event of vent issues.
...
book-text-atmos-waste = The waste network is the primary system responsible for keeping the air on the ponyville free of contaminants.

        You can identify the relevant pipes by their Pleasing Dull Red color or by using a tray scanner to trace which pipes are connected to the scrubbers on the ponyville.
...
book-text-atmos-vents =
    Below is a quick reference guide to several atmospheric devices:
                ...
                Active Vents:
                These are the most common vents on the ponyville. They have an internal pump, and require power. By default, they will only pump gases out of pipes, and only up to 101 kpa. However, they can be reconfigured using an air alarm. They will also lock out if the room is under 1 kpa, to prevent pumping gasses into space.
