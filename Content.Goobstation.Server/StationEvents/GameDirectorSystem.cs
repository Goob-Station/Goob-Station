...
public sealed class GameDirectorSystem : GameRuleSystem<GameDirectorComponent>
{
    ...
    private static readonly Gauge EventsRunTotal = Prometheus.Metrics.CreateGauge(
        "game_director_events_run_total",
        "Total number of station events run by the Game Director.",
        "event_name");

    private static readonly Gauge StoryBeatChangesTotal = Prometheus.Metrics.CreateGauge(
    ...
    private static readonly Gauge ActiveGhosts = Prometheus.Metrics.CreateGauge(
        "game_director_active_ghosts",
        "Current number of active ghosts counted by the Game Director.");

    private static readonly Gauge RoundstartAntagsSelectedTotal = Prometheus.Metrics.CreateGauge(
