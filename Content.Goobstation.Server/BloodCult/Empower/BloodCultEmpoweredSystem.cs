...
namespace Content.Goobstation.Server.BloodCult.Empower;
...
public sealed class BloodCultEmpoweredSystem : EntitySystem
{
    ...
    private void OnEmpowerStartup(Entity<BloodCultEmpoweredComponent> cultist, ref ComponentStartup args)
    {
        ...
        if (TryComp(cultist, out Spells.BloodCultSpellsHolderComponent? spellsHolder))
    ...
    private void UpdateTimers(float frameTime)
    {
        ...
        {
            if (!HasComp<Goobstation.Shared.BloodCult.BloodCultist.BloodCultistComponent>(uid))
            {
