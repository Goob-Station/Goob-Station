        ...
        bool addLog = true)
    {
        ...
        else
        {
            ...
            var logImpact = (alertMinExplosionIntensity > -1 && totalIntensity >= alertMinExplosionIntensity)
                ...
                : LogImpact.High;
            if (posFound)
                _adminLogger.Add(LogType.Explosion, logImpact, $"{ToPrettyString(user.Value):user} спричинив вибух {ToPrettyString(uid):entity} ({typeId}) у Pos:{gridPos:coordinates} з інтенсивністю {totalIntensity} та спадом {slope}");
            else
                _adminLogger.Add(LogType.Explosion, logImpact, $"{ToPrettyString(user.Value):user} спричинив вибух {ToPrettyString(uid):entity} ({typeId}) у Pos:[Grid or Map not found] з інтенсивністю {totalIntensity} та спадом {slope}");
        }
    }
