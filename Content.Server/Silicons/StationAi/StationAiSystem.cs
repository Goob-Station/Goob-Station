...
public sealed partial class StationAiSystem : SharedStationAiSystem
{
    ...
    [Dependency] private readonly Content.Server.Power.EntitySystems.BatterySystem _battery = default!; // Goob: no SharedBatterySystem in this engine
    ...
    private void OnContainerSpawn(Entity<StationAiCoreComponent> ent, ref ContainerSpawnEvent args)
    {
        ...
        {
            _battery.SetCharge(ent.Owner, battery.MaxCharge, battery); // Goob: older battery API
        }

        if (TryComp<DamageableComponent>(ent, out var coreDamageable))
            _damageable.SetAllDamage(ent.Owner, coreDamageable, 0); // Goob: older damageable API
    }
    ...
    private void UpdateDamagedAccent(Entity<StationAiCoreComponent> ent)
    {
        ...
        if (TryComp<BatteryComponent>(ent, out var battery))
            accent.OverrideChargeLevel = battery.MaxCharge > 0 ? battery.CurrentCharge / battery.MaxCharge : 0f; // Goob: older battery API

        if (TryComp<DamageableComponent>(ent, out var damageable))
            accent.OverrideTotalDamage = damageable.TotalDamage; // Goob: older damageable API
    ...
    private void UpdateBatteryAlert(Entity<StationAiCoreComponent> ent)
    {
        ...
        var chargePercent = battery.MaxCharge > 0 ? battery.CurrentCharge / battery.MaxCharge : 0f; // Goob: older battery API
    ...
    private void UpdateCoreIntegrityAlert(Entity<StationAiCoreComponent> ent, bool damageIncreased = false)
    {
        ...
        var damagePercent = damageable.TotalDamage / _destructible.DestroyedAt(ent, destructible); // Goob: older damageable API
