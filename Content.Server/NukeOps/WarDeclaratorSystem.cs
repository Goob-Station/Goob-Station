...
public sealed class WarDeclaratorSystem : EntitySystem
{
    ...
    [Dependency] private readonly AnnouncerSystem _announcer = default!; // Goobstation - Announcer System
    ...
    private void OnActivated(Entity<WarDeclaratorComponent> ent, ref WarDeclaratorActivateMessage args)
    {
        ...
        if (ev.Status == WarConditionStatus.WarReady)
        {
            ...
            _announcer.SendAnnouncement("war", Filter.Broadcast(), ent.Comp.Message, title, ent.Comp.Color); // Goobstation - Custom Announcers
