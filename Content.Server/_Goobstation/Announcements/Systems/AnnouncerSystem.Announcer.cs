...
public sealed partial class AnnouncerSystem
{
    ...
    {
        var announcer = _config.GetCVar(GoobCCVars.Announcer);
    ...
    {
        var list = _proto.Index<WeightedRandomPrototype>(_config.GetCVar(GoobCCVars.AnnouncerList));
        var blacklist = _config.GetCVar(GoobCCVars.AnnouncerBlacklist).Split(',').Select(a => a.Trim()).ToList();
