...
public sealed partial class ImplanterSystem : SharedImplanterSystem
{
    ...
    private void OnImplanterAfterInteract(EntityUid uid, ImplanterComponent component, AfterInteractEvent args)
    {
        ...
        else
        {
            ...
            if (args.User == target && _skills.HasSkill(args.User, Skills.Surgery)) // Corvax-Skills
    ...
    {
        var delay = component.ImplantTime * (!_skills.HasSkill(user, Skills.Surgery) ? ImplantDelayModifierWithoutSkill : 1); // Corvax-Skills

        var args = new DoAfterArgs(EntityManager, user, delay, new ImplantEvent(), implanter, target: target, used: implanter) // Corvax-Skills
        {
            BreakOnDamage = true,
            BreakOnMove = true,
            NeedHand = true,
    ...
    {
        var delay = component.DrawTime * (!_skills.HasSkill(user, Skills.Surgery) ? DrawDelayModifierWithoutSkill : 1); // Corvax-Skills

        var args = new DoAfterArgs(EntityManager, user, delay, new DrawEvent(), implanter, target: target, used: implanter) // Corvax-Skills
        {
            BreakOnDamage = true,
            BreakOnMove = true,
            NeedHand = true,
