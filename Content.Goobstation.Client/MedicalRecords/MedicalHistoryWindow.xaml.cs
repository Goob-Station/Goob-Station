...
public sealed partial class MedicalHistoryWindow : FancyWindow
{
    ...
    public MedicalHistoryWindow(uint maxLength)
    {
        ...
        AddButton.OnPressed += _ =>
        {
            ...
            var field = "line";
            var prompt = Loc.GetString("medical-records-console-history-message");
    ...
    public void UpdateHistory(MedicalRecord record, bool access)
    {
        ...
        foreach (var entry in record.History)
        {
            ...
            var line = time.HasValue ? $"{time.Value.Hours:00}:{time.Value.Minutes:00}:{time.Value.Seconds:00} - {entry.Message}"
                            : $"{Loc.GetString("medical-history-time-unknown")} - {entry.Message}";
