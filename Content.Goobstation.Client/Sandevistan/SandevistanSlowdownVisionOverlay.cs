...
public sealed class SandevistanSlowdownVisionOverlay : Overlay
{
    ...
    protected override bool BeforeDraw(in OverlayDrawArgs args)
    {
        return args.Viewport.Eye != null
            ...
            && _entMan.HasComponent<SandevistanSlowdownVisionComponent>(player);
    }

    protected override void Draw(in OverlayDrawArgs args)
    {
        if (args.Space == OverlaySpace.BeforeLighting)
            DrawLighting(in args);
        else
            DrawSpeed(in args);
    }
    ...
    private void DrawLighting(in OverlayDrawArgs args)
    {
        ...
        var localMatrix = viewport.LightRenderTarget.GetWorldToLocalMatrix(eye, newScale);
