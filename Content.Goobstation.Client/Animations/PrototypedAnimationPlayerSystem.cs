...
namespace Content.Goobstation.Client.Animations;
    ...
    public void PlayAnimation(EntityUid entityUid, AnimationPrototype animationPrototype)
    {
        ...
        foreach (var track in animationPrototype.Tracks)
        {
            ...
            else if (track is AnimationTrackPlaySoundData)
            {
                ...
                foreach (var keyFrame in track.KeyFrames)
                {
                    if (keyFrame is not KeyFrameSoundData data)
                        continue;

                    trackData.KeyFrames.Add(new AnimationTrackPlaySound.KeyFrame(_audio.ResolveSound(data.Sound), data.Keyframe));
                }
