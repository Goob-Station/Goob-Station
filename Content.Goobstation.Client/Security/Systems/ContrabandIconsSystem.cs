...
public sealed class ContrabandIconsSystem : SharedContrabandIconsSystem
{
    ...
    {
        if (!_isEnabled
            || !MetaData(args.Equipee)
                .EntityInitialized) // stupid fucking equip event during intialization breaks ID acquisition
            return;
        if (args.SlotFlags ==
            SlotFlags.IDCARD) // when something happens with the pda slot we need to recheck everything
        {
    ...
    private void OnEquipHands(EntityUid uid, VisibleContrabandComponent comp, DidEquipHandEvent args)
    {
        if (!_isEnabled
            || !MetaData(args.User)
                .EntityInitialized // stupid fucking hands event during intialization breaks ID acquisition
