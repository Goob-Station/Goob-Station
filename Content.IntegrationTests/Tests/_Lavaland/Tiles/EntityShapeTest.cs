...
[TestFixture]
[TestOf(typeof(EntityShapeSystem))]
public sealed class EntityShapeTest
{
    ...
    public async Task LaunchAndShutdownMegafauna()
    {
        ...
        EntityShapeSystem system = null;

        await server.WaitPost(() =>
        {
            system = entSysMan.GetEntitySystem<EntityShapeSystem>();
        });
        ...
        await server.WaitPost(() =>
        {
            ...
            {
                if (!protoMan.TryIndex<EntityShapePrototype>(shapeId, out var shapeProto))
                    continue;
