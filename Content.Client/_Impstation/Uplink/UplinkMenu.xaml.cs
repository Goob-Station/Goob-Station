...
public sealed partial class UplinkMenu : FancyWindow
{
    ...
    private void AddListingGui(ListingData listing)
    {
        ...
        var hasBalance = HasListingPrice(Balance, listing.Cost);
    ...
    private string? GetExtraText(ListingData listing)
    {
        if (listing.ProductHereticKnowledge != null)
        {
            ...
            return Loc.GetString("store-ui-heretic-level-display", ("power", knowledgeProto.Stage));
        }
    ...
    private string GetDiscountString(ListingData listingData)
    {
        if (listingData.DiscountValue <= 0)
            return string.Empty;

        var discountMessage = Loc.GetString(
            "store-ui-discount-display",
            ("amount", (listingData.DiscountValue.ToString("P0")))
        );
    ...
    private bool HasListingPrice(Dictionary<ProtoId<CurrencyPrototype>, FixedPoint2> currency, Dictionary<ProtoId<CurrencyPrototype>, FixedPoint2> price)
    {
