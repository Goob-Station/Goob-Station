...
public sealed class MobPhaseSpriteSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeNetworkEvent<MobPhaseChangedNetworkEvent>(OnPhaseChangedNetwork);
    }
    ...
    private void OnPhaseChanged(Entity<MobPhaseSpriteComponent> ent, ref MobPhaseChangedEvent args)
    {
        ...
        ApplyPhaseSprite(ent.Owner, sprite, data);
    }

    private void OnPhaseChangedNetwork(MobPhaseChangedNetworkEvent ev, EntitySessionEventArgs args)
    {
        var uid = GetEntity(ev.Entity);

        if (!TryComp(uid, out MobPhaseSpriteComponent? spriteComp))
            return;

        if (!spriteComp.Phases.TryGetValue(ev.NewPhase, out var data))
            return;

        if (!TryComp(uid, out SpriteComponent? sprite))
            return;

        ApplyPhaseSprite(uid, sprite, data);
    }

    private void ApplyPhaseSprite(EntityUid uid, SpriteComponent sprite, PhaseSpriteData data)
    {
        ...
        if (!string.IsNullOrWhiteSpace(data.Rsi))
        {
            ...
            _spriteSystem.QueueUpdateIsInert(new(uid, sprite));
        }
