...
public sealed partial class PlantAnalyzerWindow : FancyWindow
{
    ...
    public void Populate(PlantAnalyzerScannedSeedPlantInformation msg)
    {
        ...
        StringBuilder exudeGases = GetGasNamesFromFlags(msg.ExudeGases);
        StringBuilder consudeGases = GetGasNamesFromFlags(msg.ConsumeGases);
    ...
    private StringBuilder GetGasNamesFromFlags(GasFlags flags)
    {
        StringBuilder output = new();
        foreach (Gas gas in Enum.GetValues(typeof(Gas)))
        {
            string flagName = gas.ToString();
            if (Enum.TryParse<GasFlags>(flagName, out var targetFlag))
            {
                if (flags.HasFlag(targetFlag))
                {
                    var gasCleanName = gas.ToString().ToLower();
                    output.Append(IndentedNewline);
                    output.Append(Loc.GetString($"gases-{gasCleanName}"));
                }
            }
        }
        return output;
