...
using Content.Shared._Goobstation.Announcements.Events;
...
namespace Content.Client._Goobstation.Announcements.Systems;
...
public sealed class AnnouncerSystem : SharedAnnouncerSystem
{
    ...
    public override void Initialize()
    {
        ...
        AnnouncerVolume = _config.GetCVar(GoobCCVars.AnnouncerVolume) * 100f / ContentAudioSystem.AnnouncerMultiplier;

        _config.OnValueChanged(GoobCCVars.AnnouncerVolume, OnAnnouncerVolumeChanged);
        _config.OnValueChanged(GoobCCVars.AnnouncerDisableMultipleSounds, OnAnnouncerDisableMultipleSounds);
    ...
    public override void Shutdown()
    {
        ...
        _config.UnsubValueChanged(GoobCCVars.AnnouncerVolume, OnAnnouncerVolumeChanged);
        _config.UnsubValueChanged(GoobCCVars.AnnouncerDisableMultipleSounds, OnAnnouncerDisableMultipleSounds);
    }
    ...
    private void OnAnnouncementReceived(AnnouncementSendEvent ev)
    {
        ...
        if (_config.GetCVar(GoobCCVars.AnnouncerDisableMultipleSounds))
        {
            foreach (var audioSource in AnnouncerSources.ToList())
            {
                audioSource.Dispose();
