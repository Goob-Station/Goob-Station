...
public sealed partial class AACWindow : FancyWindow
{
    ...
    public void LanguageButtonRefresh(List<ProtoId<LanguagePrototype>> languages, string selectedLanguage)
    {
        ...
        {
            LanguageButton.AddItem("None");
            return;
        }

        // if there was no languagespeakercomponent at the time ui was made it is ensured therefore itll have universal.
        var hasUniversal = false;
        foreach (var lang in languages)
        {
            if (!_prototype.TryIndex(lang, out var proto))
                continue;
            if (!proto.ID.Equals("Universal", StringComparison.OrdinalIgnoreCase))
                continue;
            hasUniversal = true;
            break;
        }
