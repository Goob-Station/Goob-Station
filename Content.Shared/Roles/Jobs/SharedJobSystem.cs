...
public abstract class SharedJobSystem : EntitySystem
{
    ...
    // Goidastation Change: Returns the amount of Goidacoins a player will receive when they finish a round as this job.
    public int GetJobGoidacoins(ICommonSession player)
    {
        if (_playerSystem.ContentData(player) is not { Mind: { } mindId }
            || !MindTryGetJob(mindId, out var prototype))
            return 1;

        return prototype.Goidacoins;
    }
}