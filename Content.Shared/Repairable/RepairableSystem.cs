...
public sealed partial class RepairableSystem : EntitySystem
{
    ...
    private void OnRepairFinished(Entity<RepairableComponent> ent,  ref RepairFinishedEvent args)
    {
        ...
        // [WOUNDMED] - Repair/Heal IPC's
        var ev0 = new IPCRepairableFinishedEvent(ent, args);
        RaiseLocalEvent(ref ev0);
        // End
    ...
    private void Repair(Entity<RepairableComponent> ent, ref InteractUsingEvent args)
    {
        ...
        // [WOUNDMED] Return early if there is nothing to heal/repair
        var ev = new IPCRepairableCheckEvent(ent, args);
        RaiseLocalEvent(ref ev);
        if (!ev.AnythingToHeal)
            return;
        // End
