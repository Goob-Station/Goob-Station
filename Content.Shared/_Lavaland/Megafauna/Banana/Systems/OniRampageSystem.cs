...
namespace Content.Shared._Lavaland.Megafauna.Banana.Systems;

/// <summary>
/// This system handles jumping towards a target, dealing damage in AoE, spawning a ring of entities, and optionally playing a voiceline.
/// </summary>
public sealed class OniRampageSystem : EntitySystem
{
    ...
    private void OnLand(Entity<OniRampageComponent> ent, ref LandEvent args)
    {
        ...
        SpawnLandingRing(uid, comp, range: (int) MathF.Ceiling(range));

        if (comp.ShockwaveSound is not null)
        {
            _audio.PlayPvs(comp.ShockwaveSound, uid);
        }
    }

    private void SpawnLandingRing(EntityUid center, OniRampageComponent comp, int range)
    {
        ...
        var innerTiles = range > 1 ? new HashSet<TileRef>(_map.GetLocalTilesIntersecting(center, grid, inner)) : new HashSet<TileRef>();
        ...
        foreach (var tile in outerTiles)
        {
            ...
            Spawn(comp.RingPrototype, _map.GridTileToWorld(gridEnt.Item1, grid, tile.GridIndices));
        }
    }
