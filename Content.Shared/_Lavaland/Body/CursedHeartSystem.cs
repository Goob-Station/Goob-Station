...
public sealed class CursedHeartSystem : EntitySystem
{
    ...
    private void OnPump(EntityUid uid, CursedHeartComponent comp, PumpHeartActionEvent args)
    {
        ...
        _damage.TryChangeDamage(uid, new DamageSpecifier(_proto.Index<DamageGroupPrototype>("Brute"), -5), true, false, partMultiplier: 11f, targetPart: TargetBodyPart.All); // Shitmed Change
        _damage.TryChangeDamage(uid, new DamageSpecifier(_proto.Index<DamageGroupPrototype>("Airloss"), -5), true, false, partMultiplier: 11f, targetPart: TargetBodyPart.All); // Shitmed Change
