...
public sealed class CloakSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var comp))
        {
            ...
            if (_timing.CurTime >= comp.EndTime)
            {
                ...
                _popup.PopupPredicted(Loc.GetString("wraith-voidhound-cloak-reappear"), uid, uid);
                comp.IsActive = false;
                Dirty(uid, comp);
            }
        }
    ...
    private void OnCloak(Entity<CloakComponent> ent, ref CloakEvent args)
    {
        ...
        _popup.PopupPredicted(Loc.GetString("wraith-voidhound-cloak-slip"), ent.Owner, ent.Owner);
