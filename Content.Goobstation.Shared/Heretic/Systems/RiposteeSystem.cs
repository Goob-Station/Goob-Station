...
public sealed class RiposteeSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<RiposteeComponent, BeforeHarmfulActionEvent>(OnHarmAttempt,
            before: [typeof(SharedHereticAbilitySystem)]);
    ...
    {
        if (_net.IsServer
            || !TryGetEntity(ev.User, out var user)
            || !TryGetEntity(ev.Target, out var target)
            || !TryGetEntity(ev.Weapon, out var weapon)
            || _player.LocalEntity != user.Value
            || !TryComp<MeleeWeaponComponent>(weapon.Value, out var melee)
            || !TryComp<RiposteeComponent>(user.Value, out var ripostee))
            return;
    ...
    public override void Update(float frameTime)
    {
        ...
        var query = EntityQueryEnumerator<RiposteeComponent>();
        while (query.MoveNext(out var uid, out var rip))
        {
            if (!uid.IsValid())
                continue;
    ...
    private void OnHarmAttempt(Entity<RiposteeComponent> ent, ref BeforeHarmfulActionEvent args)
    {
        if (args.Cancelled
            ...
            || HasComp<RiposteeComponent>(args.User))
            return;

        foreach (var data in ent.Comp.Data.Values
                     .Where(data => data.CanRiposte))
        {
            ...
            if (weapon == null
                || !_blocker.CanAttack(ent, args.User, weapon.Value))
                continue;
