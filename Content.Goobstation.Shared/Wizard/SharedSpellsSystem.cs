...
public abstract class SharedSpellsSystem : EntitySystem
{
    ...
    private void OnSwapSecondaryTarget(SetSwapSecondaryTarget ev)
    {
        ...
        if (!TryComp(action, out Components.SwapSpellComponent? swap))
            return;
    ...
    private void OnMutate(MutateSpellEvent ev)
    {
        ...
        EnsureComp<Mutate.HulkComponent>(ev.Performer).Duration = ev.Duration;
    ...
    private void OnSpellCards(SpellCardsEvent ev)
    {
        ...
        if (!TryComp(ev.Action.Owner, out SpellCards.SpellCardsActionComponent? spellCardsAction))
            return;
    ...
    private void OnLesserSummonGuns(LesserSummonGunsEvent ev)
    {
        ...
        var comp = EnsureComp<LesserSummonGuns.EnchantedBoltActionRifleComponent>(gun.Value);
    ...
    private void OnInstantSummons(InstantSummonsEvent ev)
    {
        ...
        if (!TryComp(ev.Action, out Components.InstantSummonsActionComponent? summons))
            return;
    ...
    private void OnTraps(TrapsSpellEvent ev)
    {
        ...
        bool IsTileValid((EntityCoordinates, TileRef) data)
        {
            ...
            var trapQuery = GetEntityQuery<Traps.WizardTrapComponent>();
        ...
        for (var i = 0; i < Math.Min(tiles.Count, ev.Amount); i++)
        {
            ...
            var trapComp = EnsureComp<Traps.WizardTrapComponent>(trap);
    ...
    private void OnExsangunatingStrike(ExsanguinatingStrikeEvent ev)
    {
        ...
        if (HasComp<SanguineStrike.SanguineStrikeComponent>(item))
        {
        ...
        AddComp<SanguineStrike.SanguineStrikeComponent>(item);
    ...
    private void OnChuuniInvocations(ChuuniInvocationsEvent ev)
    {
        ...
        if (_inventory.TryGetSlotEntity(ev.Performer, "eyes", out var eyepatch, inventory) &&
            HasComp<Chuuni.ChuuniEyepatchComponent>(eyepatch.Value))
        {
    ...
    private void OnSwap(SwapSpellEvent ev)
    {
        ...
        if (!TryComp(ev.Action, out Components.SwapSpellComponent? swap))
            return;
    ...
    private void OnPredictionToggle(PredictionToggleSpellEvent ev)
    {
        ...
        if (HasComp<CurseOfByond.CurseOfByondComponent>(ev.Target))
            RemComp<CurseOfByond.CurseOfByondComponent>(ev.Target);
        else
            EnsureComp<CurseOfByond.CurseOfByondComponent>(ev.Target);
    ...
    private bool ValidateLockOnAction(EntityWorldTargetActionEvent ev)
    {
        ...
        if (!TryComp(ev.Action.Owner, out SpellCards.LockOnMarkActionComponent? lockOnMark))
            return false;
