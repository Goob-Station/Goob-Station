...
public sealed class FreezeContactsSystem : EntitySystem
{
    ...
    private void OnRemove(Entity<FrozenComponent> ent, ref ComponentRemove args)
    {
        ...
        if (TryComp(uid, out FadingTimedDespawn.FadingTimedDespawnComponent? fading) && !fading.FadeOutStarted)
    ...
    private void OnEntityEnter(EntityUid uid, FreezeContactsComponent component, ref StartCollideEvent args)
    {
        ...
        FadingTimedDespawn.FadingTimedDespawnComponent? fading;
        ...
        if (IsImmune(otherUid) || TryComp(otherUid, out Guardian.GuardianSharedComponent? guardian) && IsImmune(guardian.Host))
            return;
