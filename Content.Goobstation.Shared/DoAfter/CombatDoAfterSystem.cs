...
public sealed partial class CombatDoAfterSystem : EntitySystem
{
        ...
        CombatDoAfterComponent? combatDoAfter = null)
    {
        ...
        if (combatDoAfter is { DoAfterUser: not null, DoAfterId: not null })
        {
            _doAfter.Cancel(combatDoAfter.DoAfterUser.Value, combatDoAfter.DoAfterId.Value);
            combatDoAfter.DoAfterUser = null;
            combatDoAfter.DoAfterId = null;
            dirty = false;
    ...
    private bool CheckDoAfter(Entity<CombatDoAfterComponent> item, EntityUid? user)
    {
