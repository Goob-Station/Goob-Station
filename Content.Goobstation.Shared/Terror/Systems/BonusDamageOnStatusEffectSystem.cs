...
public sealed class BonusDamageOnStatusEffectSystem : EntitySystem
{
    ...
    private void OnMeleeHit(Entity<BonusDamageOnStatusEffectComponent> ent, ref MeleeHitEvent args)
    {
        ...
        {
            var hasRequiredEffect = false;

            foreach (var effect in ent.Comp.RequiredStatusEffects)
            {
                if (!_status.HasStatusEffect(target, effect)) continue;

                hasRequiredEffect = true;
                break;
            }

            if (!hasRequiredEffect) continue;
